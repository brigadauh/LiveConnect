...
gulp.task('serve', ['html', 'styles', 'scripts','images','misc'], function() {
    const target='http://192.168.1.3:8888';
  ...
  browserSync.init({
    ...
    middleware: function(req, res, next) {
      if (req.url.indexOf('/api/') !== -1) {
        var p = proxy.web(req, res,{target: target});
        return;
      }
  ...
  gulp.watch(PATH.src.baseDir+"/*.html").on('change', browserSync.reload);
  gulp.watch(PATH.src.baseDir+"/*.nunjucks").on('change', browserSync.reload);
